...
class RegisterForm(forms.Form):
    ...
    password = forms.CharField(label='Секретный ключ', widget=forms.PasswordInput(attrs={
        'class': 'form-control',
        'placeholder': 'Введите секретный ключ'
    }))
    password2 = forms.CharField(label='Повторите секретный ключ', widget=forms.PasswordInput(attrs={
        'class': 'form-control',
    ...
    def clean_password2(self):
        ...
        if password != password2:
            raise ValidationError('Кокуш твои пароли не совпадают')
        return password
    ...
    def save(self, commit=True):
        ...
        send_mail(
            ...
            message='письмо потверждение регистрации',
            html_message='<a href="http://localhost:8000/confirm/?code=%s">Confirm please</a>'%code,
